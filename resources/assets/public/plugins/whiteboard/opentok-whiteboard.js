...
var OpenTokWhiteboard = ng.module('opentok-whiteboard', ['opentok'])
    .directive('otWhiteboard', ['OTSession', '$window', function (OTSession, $window) {
        return {
            restrict: 'E',
            template: '<canvas hidpi="off" ng-class="{eraser_selected: erasing}"></canvas>' +

            '<div class="panel-tool hidden"><div class="open-tool"><span class="fa fa-pencil"></span></div><div class="OT_panel">' +
            ...
            '<div title="Eraser" ng-click="erase()" ng-class="{OT_erase: true, OT_selected: erasing}" class="OT_button">' +
            '<span class="fa fa fa-eraser"></span></div>' +
            '<div title="Undo" ng-click="undo()" class="OT_button OT_capture OT_undo" value="Undo"><span class="fa fa-mail-reply"></span></div>' +
            '<div title="Redo" ng-click="redo()" class="OT_button OT_capture OT_redo" value="Redo"><span class="fa fa-mail-forward"></span></div>' +
            '<div title="Clear" ng-click="clear()" class="OT_button OT_clear" value="Clear"><span class="fa fa-trash"></span></div>' +
            '</div></div>',
            ...
            link: function (scope, element, attrs) {
                ...
                scope.changeColor(scope.colors[isTeacher !== '' ? 1 : 0]);
                ...
                $("#zoomOut").click(function(){
                    ...
                    sendUpdate('pdf_zoom_out', update);
                });
                $("#tmpOnloadPdf").click(function(){
                    var pdfUrl = $('#cPdfUrl').val();
                    var update = {
                        id: OTSession.session && OTSession.session.connection &&
                        OTSession.session.connection.connectionId,
                        uuid: client.uuid,
                    ...
                    sendUpdate('pdf_load', update);
                    clearCanvas();
                });
                $("#next").click(function(){
                    var jsonHistory = JSON.stringify(drawHistory);
                    whiteBoard.setHistory(CURRENT_PAGE, jsonHistory);
                    whiteBoard.nextPage();
                    clearCanvas();
                    var history = whiteBoard.getHistory(CURRENT_PAGE);
                    if(history !== ""){
                        var objHistory = JSON.parse(history);
                        var arr = [];
                        for(var x in objHistory){
                            arr.push(objHistory[x]);
                        }
                        drawUpdates(arr);
                    }
                    if (OTSession.session) {
                ...
                $("#scaleSelect").on("change",function(){
                    var update = {
                        ...
                        OTSession.session.connection.connectionId,
                        uuid: client.uuid,
                        event: 'pdf_scale_change',
                        scale: $(this).val()
                    };
                    sendUpdate('pdf_scale_change', update);
                });
                ...
                if (OTSession.session) {
                    ...
                    OTSession.session.on({
                        ...
                        'signal:pdf_load':function(event){
                            if (event.from.connectionId !== OTSession.session.connection.connectionId) {
                                var objData = JSON.parse(event.data);
                                clearCanvas();
                                whiteBoard.loadPdf(objData[0].url);
								fileId = objData[0].file_id;
                            }
                        },
                        'signal:pdf_goto':function(event){
                            if (event.from.connectionId !== OTSession.session.connection.connectionId) {
                                var objData = JSON.parse(event.data);
                                whiteBoard.goToPage(objData[0].page);
                                clearCanvas();
                                var history = whiteBoard.getHistory(CURRENT_PAGE);
                                if(history !== ""){
                                    var objHistory = JSON.parse(history);
                        ...
                        'signal:otWhiteboard_pdf_history': function (event) {
                            //Load pdf when open
                            if (event.from.connectionId !== OTSession.session.connection.connectionId) {
                                var objData = JSON.parse(event.data);
                                whiteBoard.loadPdf(objData[0].pdf_url);
								fileId = objData[0].file_id;
                            }
                        },
                        ...
                        'signal:otWhiteboard_request_history': function (event) {
							...
							if(pdfUrl !== ""){
								...
								sendUpdate('otWhiteboard_pdf_history', update);
							}
                        }
                    });
                }
